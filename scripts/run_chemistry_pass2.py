#!/usr/bin/env python3
"""Execute the Chemistry two-pass run (Pass 1 rebuild + Pass 2 synthesis) and log it.
...
under ``runs/<run_id>/``:
    ...
    scripts/build_chemistry_pass1.py   Pass 1 - per-paper evidence extraction
    scripts/build_chemistry_pass2.py   Pass 2 - families, models, breakdowns, diagnostics
...
organized sample contains no Grade 11 Chemistry curriculum document.

Usage:  python scripts/run_chemistry_pass2.py
"""
...
SUBJECT = "chemistry"
ORGANIZED = ROOT / "data" / "organized" / "chemistry"
REVIEW_QUEUE = ROOT / "review_queue" / "RQ-P4-CHEM-PASS2.yaml"
...
RESOLUTIONS = {
    ...
    "curriculum_gap": [
        "acquire the Grade 11 Chemistry curriculum / ATP document",
    ...
    "provenance_gap": [
        ...
        "map each chemistry_{...}_q{qn} source_id to a real SOURCE-* id and re-point the provenance chain",
        "continue to carry the sentinel and document the gap until ORC runs",
    ],
    "image_dependent": [
        "verify each image-dependent member against the source PDF before production use",
        "cap the family/model at medium confidence while the diagram is unverified",
        "drop the image-dependent member from the diagnostic bank",
    ],
    "marking_anomaly": [
        "confirm against a clean copy of the memorandum if one becomes available",
        "keep the question-paper-authoritative resolution already applied in Pass 1",
    ],
    "visual_only_item": [
        "recover the source images and transcribe the structures before use",
        "keep the item requires_visual_verification=True and do not assert a structural claim",
    ],
}
...
def corpus_counts() -> dict[str, int]:
    """Count what is physically present in the organized chemistry sample."""
    ...
    return {
        ...
        "memoranda_in_sample": len(files) - len(qp_named),
    }
...
def write_review_queue(run_id: str, unresolved: list[dict]) -> None:
    ...
    lines = [
        "review_id: RQ-P4-CHEM-PASS2",
        f"run_id: {run_id}",
        "subject: chemistry",
        "phase: 4-11",
        "created_by: scripts/run_chemistry_pass2.py",
...
def main() -> int:
    ...
    logger.log_event(
        "EXTRACTION_STARTED",
        message="Pass 1 re-run for chemistry from the organized source PDFs",
        metadata=corpus,
    )

    rc1 = run_builder("build_chemistry_pass1.py")
    pass1_summary = json.loads((ROOT / "data/extracted/pass1/chemistry/_BATCH_SUMMARY.json").read_text())
    if rc1 != 0 or pass1_summary["problems"]:
        logger.log_error(
            ...
            affected_artifact="data/extracted/chemistry_pass1.json",
            status="unresolved",
        )
    else:
        logger.log_event(
    ...
    logger.log_decision(
        ...
        context="The inherited data/extracted/chemistry_pass1.json (if any) could not be trusted as citable "
                "per-question evidence; Pass 2 requires real question records, so Pass 1 was rebuilt from the "
                "organized source PDFs.",
        evidence=["scripts/build_chemistry_pass1.py", "data/organized/chemistry/"],
        alternatives=["run Pass 2 on a hollow batch and report zero families",
                      "rebuild Pass 1 from the source PDFs, then run Pass 2"],
        selected="rebuild Pass 1 from the source PDFs, then run Pass 2",
        rationale="The PDFs in data/organized/chemistry are tracked in git and have a clean text layer "
                  "(verified with pypdf), so every Pass 2 claim rests on a real question record with a "
    ...
    logger.log_decision(
        ...
        context="data/organized/sample_manifest.json must not be trusted for chemistry paper/memo pairing.",
        evidence=["scripts/build_chemistry_pass1.py verify_alignment()",
    ...
    logger.log_decision(
        ...
        context="Pass 2 says confidence is 'high' with 4+ member questions across different papers, "
                "'medium' with 2-3, and never 'high' if all support comes from one paper.",
        evidence=["scripts/build_chemistry_pass2.py compute_confidence()"],
    ...
    logger.log_decision(
        ...
        context="Each extracted question record is assigned to at most one family.",
        evidence=["scripts/build_chemistry_pass2.py partition check"],
        ...
        rationale="Overlap would let one question inflate several families and make frequency counts "
                  "meaningless. Records fitting no family are reported unassigned (UNRES-CHEM-021) "
    ...
    logger.log_decision(
        ...
        context="Pass 2 names family fields that database/schema/question_family.schema.json does not have "
        ...
        alternatives=["relax the repo schema to accept the prompt's field names",
                      "map the prompt's fields onto the repo schema"],
        selected="map onto the repo schema: member_source_ids -> source_evidence, distinguishing_features -> "
                 "definition + assessment_operations + topics, typical_marks_range -> _PASS2_SUMMARY.json",
        rationale="The persisted object must validate against the repository schema, which IMPLEMENTATION_SPEC "
                  "Phase 8 makes authoritative.",
        confidence="high",
        requires_review=True,
    )
    logger.log_decision(
        ...
        context="Some recurring patterns meet the member minimum but rest on too few papers to generalise.",
        evidence=["ingestion/analysis/chemistry_pass2_models.py DEFERRED_FAMILIES"],
        alternatives=["promote them to families", "defer them to unresolved_items and say so"],
        selected="defer them to unresolved_items, flagged UNRES-CHEM-024",
        rationale="Their marking_requirements could not be generalised from the evidence. Nothing is hidden - "
                  "it is in unresolved_items.json with its evidence, so promotion is a one-line change.",
        confidence="medium",
        requires_review=True,
    )
    logger.log_decision(
        decision_id="DEC-P4-007",
        decision_type="orc_provenance_sentinel",
        context="Chemistry Phase 2 (ORC) never ran and no data/raw/chemistry/SOURCE_INVENTORY.yaml exists, "
                "so no SOURCE-* id can anchor the Pass 1 records.",
        evidence=["data/raw/chemistry/SOURCE_INVENTORY.yaml (absent)",
                  "SUBJECT_PROMPTS.md: do not invent ORC ids or a fake inventory"],
        alternatives=["invent a SOURCE-CHEM-* inventory to satisfy the schema",
                      "carry the sentinel SOURCE-ORC-CHEM-NOT-INDEXED and record the gap"],
        selected="carry the sentinel SOURCE-ORC-CHEM-NOT-INDEXED and record the gap (UNRES-CHEM-ACQ-001)",
        rationale="AGENTS.md and SUBJECT_PROMPTS.md forbid fabricating provenance. The referential chain "
                  "chemistry_{year}_{board}_{ptype}_{period}_q{qn} -> SOURCE-ORC-CHEM-NOT-INDEXED terminates "
                  "at the sentinel rather than at a real ORC id, and the gap is explicit.",
        confidence="high",
        requires_review=True,
    )

    logger.log_event("EXTRACTION_STARTED", message="Pass 2 synthesis over the chemistry batch")
    rc2 = run_builder("build_chemistry_pass2.py")
    summary = json.loads((ROOT / "knowledge/chemistry/_PASS2_SUMMARY.json").read_text())
    saturation = json.loads((ROOT / "knowledge/chemistry/saturation_report.json").read_text())
    unresolved = json.loads((ROOT / "knowledge/chemistry/unresolved_items.json").read_text())
    ...
    for model in json.loads((ROOT / "knowledge/chemistry/understanding_models.json").read_text()):
        logger.log_event(
    ...
    for b in json.loads((ROOT / "knowledge/chemistry/breakdown_models.json").read_text()):
        logger.log_event("BREAKDOWN_CREATED", entity_id=b["breakdown_id"],
                         message=f"stage={b['stage']} parent={b['parent_understanding_model']}")
    for d in json.loads((ROOT / "knowledge/chemistry/diagnostic_questions.json").read_text()):
    ...
    for err in summary["errors"]:
        logger.log_error(error_type="pass2_consistency", message=err,
                         affected_artifact="knowledge/chemistry/", status="unresolved")
    if saturation["families_first_observed_in_final_third"] > 0:
        logger.log_error(
            ...
            message="families_first_observed_in_final_third > 0 "
                    f"({saturation['families_new_in_final_third']}): chemistry cannot be declared saturated",
            affected_artifact="knowledge/chemistry/saturation_report.json",
            status="unresolved",
        )

    batch = json.loads((ROOT / "data/extracted/chemistry_pass1.json").read_text())
    marks_mismatch = sorted({r["paper_key"] for r in batch if not r["memo_alignment"]["marks_match"]})
    logger.set_metrics({
        "papers_in_organized_sample": corpus["files_in_organized_sample"],
        ...
        "memo_marks_disagreement_papers": marks_mismatch,
    ...
    for rel in [
        "data/extracted/chemistry_pass1.json",
        "data/extracted/pass1/chemistry/_BATCH_SUMMARY.json",
        "knowledge/chemistry/question_families.json",
        "knowledge/chemistry/understanding_models.json",
        "knowledge/chemistry/breakdown_models.json",
        "knowledge/chemistry/diagnostic_questions.json",
        "knowledge/chemistry/unresolved_items.json",
        "knowledge/chemistry/saturation_report.json",
        "knowledge/chemistry/pass2_output.json",
        "knowledge/chemistry/_PASS2_SUMMARY.json",
        "knowledge/chemistry/PASS2_REPORT.md",
        str(REVIEW_QUEUE.relative_to(ROOT)),
    ]:
        path = ROOT / rel
        if path.exists():
            logger.record_artifact(path, description=f"Chemistry two-pass artifact: {rel}")
