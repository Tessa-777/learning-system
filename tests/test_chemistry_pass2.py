"""Tests for the Chemistry two-pass output (Pass 1 evidence + Pass 2 synthesis).

Mirrors ``tests/test_physics_pass2.py`` in structure. Differences that follow from the
chemistry corpus:

* discipline is ``Chemistry`` (not ``Physics``);
* chemistry Phase 2 (ORC) never ran and no ``data/raw/chemistry/SOURCE_INVENTORY.yaml``
  exists, so every Pass 1 record and every model carries the sentinel
  ``SOURCE-ORC-CHEM-NOT-INDEXED`` as its ORC source id; the provenance gap is asserted
  directly (UNRES-CHEM-ACQ-001 + the review-queue ``provenance_gap`` item) rather than
  resolved against an inventory;
* the batch holds 11 papers (>= 8), so saturation is honest because strata are left
  unsampled, not because the paper count is small.
...
KNOWLEDGE = ROOT / "knowledge" / "chemistry"
...
def batch() -> list[dict]:
    return json.loads((ROOT / "data" / "extracted" / "chemistry_pass1.json").read_text(encoding="utf-8"))
...
def check_pass2_output(payloads: dict, batch: list[dict], validator) -> None:
    ...
    # -- no family silently mixes disciplines (everything is Chemistry) ------
    for fam in families:
        disciplines = {by_id[sid]["discipline"] for sid in fam["source_evidence"]}
        assert disciplines == {"Chemistry"}, f"{fam['question_family_id']} mixes {disciplines}"
    ...
    assert single_paper <= set(reported["UNRES-CHEM-022"]["affected"])
    claimed = {sid for f in families for sid in f["source_evidence"]}
    unassigned = {r["source_id"] for r in batch} - claimed
    assert unassigned == set(reported["UNRES-CHEM-021"]["affected"])
    thin = {b["breakdown_id"] for b in breakdowns if len(b["source_basis"]) < 2}
    assert thin == set(reported["UNRES-CHEM-023"]["affected"])
    assert reported["UNRES-CHEM-023"]["type"] == "single_exemplar"
    ...
    assert image_heavy == set(reported["UNRES-CHEM-011"]["affected"])
...
def test_pass1_builder_runs_clean(tmp_path):
    ...
    mod = load_builder("build_chemistry_pass1.py")
    mod.EXTRACTED = tmp_path
    mod.PER_PAPER_DIR = tmp_path / "pass1" / "chemistry"
    assert mod.main() == 0

    batch = json.loads((tmp_path / "chemistry_pass1.json").read_text(encoding="utf-8"))
    assert batch, "Pass 1 produced no records"
    per_paper = sorted((tmp_path / "pass1" / "chemistry").glob("CHE-*.json"))
    assert len(per_paper) == 11
    for path in per_paper:
        doc = json.loads(path.read_text(encoding="utf-8"))
        assert doc["marks_sum_check"]["match"], path.name
        assert "memo_alignment_check" in doc and "date_match" in doc["memo_alignment_check"], path.name
        assert doc["records_extracted"] == len(doc["question_records"]) > 0
    summary = json.loads((tmp_path / "pass1" / "chemistry" / "_BATCH_SUMMARY.json").read_text())
...
def test_pass2_builder_runs_clean(tmp_path, batch, validator):
    ...
    mod = load_builder("build_chemistry_pass2.py")
...
def test_pass1_records_are_provenanced(batch):
    ...
    for record in batch:
        ...
        assert record["discipline"] == "Chemistry"
...
def test_provenance_chain_terminates_at_sentinel(batch, knowledge):
    """RUN_LOG_SPEC 9: model -> family -> source_id -> Phase 2 ORC source.

    Chemistry Phase 2 (ORC) never ran and no inventory exists, so the chain cannot
    reach a real ORC id. It must terminate honestly at the sentinel and record the gap
    (UNRES-CHEM-ACQ-001) rather than fabricating ids.
    """
    for record in batch:
        assert record["orc_source_id"] == ORC_SENTINEL, record["source_id"]
        assert record["orc_memo_source_id"] == ORC_SENTINEL, record["source_id"]

    reported = {i["id"]: i for i in knowledge["unresolved"]}
    assert "UNRES-CHEM-ACQ-001" in reported, "provenance gap must be recorded"
    assert reported["UNRES-CHEM-ACQ-001"]["type"] == "provenance_gap"
    ...
    for model in knowledge["models"]:
        ...
        # every cited record's ORC id is the sentinel
        assert all(c == ORC_SENTINEL for c in chains), model["identity"]
        # the model's provenance chain acknowledges the sentinel / gap
        assert any(ORC_SENTINEL in p or "UNRES-CHEM-ACQ-001" in p for p in model["provenance"]), \
            f"{model['identity']} provenance does not reference the ORC sentinel/gap"
...
def test_combined_pass2_output_matches_the_per_type_files(knowledge):
    ...
    assert combined["subject"] == "chemistry"
    for key, part in [
...
def test_saturation_report_is_honest(knowledge):
    """Saturation must not be claimed while strata are left unsampled."""
    ...
    assert sat["additional_diagnostics"]["reason_not_saturated"], "reason must be stated"
    assert sat["papers_in_sample"] >= 8, "chemistry batch has 11 papers, so saturation fails on strata"


def test_review_queue_items_state_resolutions():
    """RUN_LOG_SPEC 10: issue, affected entity, evidence, resolutions, review."""
    path = ROOT / "review_queue" / "RQ-P4-CHEM-PASS2.yaml"
    assert path.exists(), "run scripts/run_chemistry_pass2.py to create the review queue"
    doc = yaml.safe_load(path.read_text(encoding="utf-8"))
    assert doc["items"]
    assert doc["item_count"] == len(doc["items"])
    ids = {item["id"] for item in doc["items"]}
    assert "UNRES-CHEM-ACQ-001" in ids, "provenance gap must be in the review queue"
    for item in doc["items"]:
        # affected_entity may be an empty list (e.g. UNRES-CHEM-022 when no family rests
        # on a single paper); it must be present, but not necessarily non-empty.
        assert "affected_entity" in item, f"{item.get('id')}: missing affected_entity"
        for field in ("issue", "type", "id", "evidence",
...
def test_run_log_was_written():
    ...
    for run in runs:
        ...
        if meta.get("subject") == "chemistry" and meta.get("phase") == "4-11":
            chemistry_runs.append((run, meta))
    assert chemistry_runs, "no chemistry two-pass run logged under runs/"
    run, meta = chemistry_runs[-1]
    assert meta["status"] in {"completed", "completed_with_review", "blocked", "failed"}
    assert meta["status"] == "completed_with_review", "this run has unresolved review items"
    ...
    assert {d["decision_id"] for d in decisions} >= {
        "DEC-P4-001", "DEC-P4-002", "DEC-P4-003", "DEC-P4-004", "DEC-P4-005",
        "DEC-P4-006", "DEC-P4-007",
    }
